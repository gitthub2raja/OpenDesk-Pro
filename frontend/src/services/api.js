...
export const paymentAPI = {
  submitUpgradeRequest: async (transactionId, screenshot) => {
    const formData = new FormData()
    ...
    const token = localStorage.getItem('token')
    const response = await fetch(`${API_BASE_URL}/payment/submit-request`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  approveUpgrade: async (organizationId, subscriptionMonths = 12) => {
    return apiCall('/payment/admin/approve-upgrade', {
      method: 'POST',
      body: JSON.stringify({ organizationId, subscriptionMonths }),
    })
  },
  getPendingUpgrades: async () => {
    return apiCall('/payment/admin/pending-upgrades')
  },
}
