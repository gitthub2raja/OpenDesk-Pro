...
router.use(protect)
// All routes require PRO plan
router.use(checkFeature('EXTERNAL_INTEGRATIONS'))
...
router.post('/', admin, async (req, res) => {
  try {
    ...
    // Check for Azure Sentinel - requires PRO plan
    if (type === 'azure-sentinel') {
      ...
      if (org && org.plan !== 'PRO') {
        return res.status(403).json({ 
          message: 'Azure Sentinel integration requires a Pro upgrade.',
